    ...
    OnMapReadyCallback
{
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        val harryRansomCenterCoordinates = LatLng(30.284378, -97.741236)
        map.moveCamera(CameraUpdateFactory.newLatLngZoom(harryRansomCenterCoordinates, 15.0f))

        // setting onclick listener for clearing map
        map.setOnMapLongClickListener {
            map.clear()
        }

        map.setOnMapClickListener {
            val lat = String.format(Locale.getDefault(), "%.3f", it.latitude)
            val long = String.format(Locale.getDefault(), "%.3f", it.longitude)
            map.addMarker(MarkerOptions().position(it).title("$lat, $long"))
        }

        // setting clear btn listener
